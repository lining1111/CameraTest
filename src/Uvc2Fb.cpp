...
int Uvc2Fb(void) {
    ...
    lcdptr = (char *) mmap(NULL, lcd_w * lcd_h * 4, PROT_READ | PROT_WRITE, MAP_SHARED, lcdfd, 0);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        mptr[i] = (unsigned char *) mmap(NULL, mapbuffer.length, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
                                         mapbuffer.m.offset);
    ...
    while (1) {
        ...
        //将mjpeg存为图片
//        FILE *file = fopen("capture.jpg","wb");
//        fwrite(mptr[readbuffer.index],1,readbuffer.length,file);
//        fclose(file);


        read_JPEG_file((const char *) mptr[readbuffer.index], (char *) rgbdata, readbuffer.length);
        printf("1212121212\n");

        //opencv 显示
//        memset(rgbdata, 0, sizeof(rgbdata));
        Mat img = Mat(480, 640, CV_8UC3, rgbdata);
        imshow("img", img);
        waitKey(20);
